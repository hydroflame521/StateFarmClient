// ==UserScript==
// @name         StateFarm Client Cyan Edition
...
XMLHttpRequest = class extends XMLHttpRequest {
	...
	get response() {
		...
		if ( this.isScript ) {
			...
			console.log( '%cInjecting code...', 'color: blue; background: black; font-size: 2em;', {
				babylonVarName,
				playersVarName,
				myPlayerVarName,
				playersVarName,
			...
			return code.replace( sceneVarName + '.render()', `( function () {
				...
				for ( let i = 0; i < players.length; i ++ ) {
					...
					if ( player.sphere === undefined ) {
						...
						material.emissiveColor = material.diffuseColor = new BABYLON.Color3( 0, 221, 255 );
					...
					if ( player.lines === undefined ) {
						...
						lines.color = new BABYLON.Color3( 0, 221, 255 );
				...
				for ( let i = 0; i < window.lines.length; i ++ ) {
					if ( ! window.lines[ i ].playerExists ) {
						console.log( '%cRemoving line...', 'color: blue; background: black; font-size: 2em;' );
...
window.addEventListener( 'DOMContentLoaded', async function () {
	...
	temp.innerHTML = '<div class="popup_window popup_lg centered roundme_lg" style="z-index: 9999999;">' +
		...
		<div id="btn-horizontal" class="f-center">
			<button class="ss_button btn_blue bevel_blue btn_sm" onclick="window.open('https://discord.gg/K24Zxy88VM', '_blank')">Discord</button>
